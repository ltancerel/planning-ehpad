...
import { creerAnnee, modifierAnnee } from "./actions";

export default function AnneesAdminClient({ annees }: { annees: AnneePlanifiee[] }) {
  ...
  function enregistrer(donnees: Omit<AnneePlanifiee, "id">) {
    demarrer(async () => {
      if (anneeEnEdition) {
        const resultat = await modifierAnnee(anneeEnEdition.id, donnees.jourDemarrage, donnees.joursFeries);
        if (resultat?.error) {
        ...
        const resultat = await creerAnnee(donnees.annee, donnees.jourDemarrage, donnees.joursFeries);
        if (resultat.error) {
          setErreur(resultat.error);
          return;
        }
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-zinc-500">
              {annees.length} année{annees.length > 1 ? "s" : ""} planifiée{annees.length > 1 ? "s" : ""}
            </p>
          </div>
          <button
